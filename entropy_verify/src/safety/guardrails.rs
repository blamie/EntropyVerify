...
/// Normalize Windows drive letter inputs and return the canonical absolute path.
pub fn normalize_and_canonicalize_target(target: &Path) -> std::io::Result<std::path::PathBuf> {
    let target_str = target.to_string_lossy().trim().to_string();
    let mut normalized = target.to_path_buf();

    #[cfg(target_os = "windows")]
    {
        // 1. Single letter e.g. "D" or "d"
        if target_str.len() == 1 {
            let c = target_str.chars().next().unwrap();
            if c.is_ascii_alphabetic() {
                normalized = std::path::PathBuf::from(format!("{}:\\", c.to_uppercase()));
            }
        }
        // 2. Drive letter with colon e.g. "D:" or "d:"
        if target_str.len() == 2 && target_str.ends_with(':') {
            let c = target_str.chars().next().unwrap();
            if c.is_ascii_alphabetic() {
                normalized = std::path::PathBuf::from(format!("{}:\\", c.to_uppercase()));
            }
    ...
    let mut canonical = std::fs::canonicalize(&normalized)?;
