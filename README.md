...
The primary goal  of EntropyVerify is to accurately verify real storage capacity while safely maximizing the physical read/write throughput of high-speed storage.
...
1. A console window opens and launches an interactive **Drive Selection Menu** inside the TUI.
2. Use the arrow keys `[↑/↓]` to cycle through available drives (shows Drive Letter, File System, Total capacity, and Free space).
3. Press `[Enter]` to select the target drive. (If a drive fails safety validation, a red error block will display inline, letting you choose another).
4. Press `[Q]` or `[Esc]` to quit.
5. The tool will calculate the maximum usable space (leaving a 100 MiB safety margin so the target drive doesn't become completely full) and initiate the main TUI dashboard.
6. Upon completion or failure, the terminal will wait for your interaction:
   ```
   Press Enter to exit...
   ```
...
| `0x05` | 1 | Padding | `u8` | Alignment padding |
| `0x06` | 2 | Thread ID | `u16` | ID of the worker thread that wrote the block |
| `0x08` | 4 | File Index | `u32` | Index of the parent file chunk |
| `0x0C` | 4 | Block Index | `u32` | Offset index of the block within the file |
| `0x10` | 16 | AES Nonce | `[u8; 16]` | Deterministic AES-CTR block nonce |
| `0x20` | 32 | Checksum | `[u8; 32]` | BLAKE3 hash of the payload section |
| `0x40` | 2,097,088 | Payload | `[u8]` | Deterministic AES-CTR pseudo-random stream |
