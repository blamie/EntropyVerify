...
/// Check if a path targets the system drive partition (Windows).
#[cfg(target_os = "windows")]
pub fn is_on_system_drive(path: &Path) -> bool {
    let path_str = path.to_string_lossy().to_uppercase();
    if let Ok(sys_drive) = std::env::var("SystemDrive") {
        let sys_prefix = format!("{}:", sys_drive.trim_end_matches(':').to_uppercase());
        path_str.starts_with(&sys_prefix)
    } else {
...
/// Check if a path targets the system root partition (Linux).
#[cfg(target_os = "linux")]
pub fn is_on_system_drive(path: &Path) -> bool {
    if let Some(vol) = get_volume_info(path) {
        vol.mount_point == Path::new("/")
    } else {
        let path_str = path.to_string_lossy();
        path_str.starts_with('/') && !path_str.starts_with("/media") && !path_str.starts_with("/mnt")
    }
}
